...
class GitAICommitter:
    ...
    def _generate_commit_prompt(self, diff_content):
       
        
        # This is the detailed prompt designed to guide the AI model
        return f"""
You are an expert software engineer and git user. Analyze the following git diff and group related changes across multiple files into logical, conventional commits. For each commit, provide:
    - "files": a list of file paths included in that commit,
    - "summary_of_changes": a clear, conventional commit message (e.g., "feat: add user authentication" or "fix: correct typo in README").

    Requirements:
    - Output a JSON object with a single key "commits", whose value is a list of commit objects.
    - Each commit object must have:
      {{
        "files": ["relative/path/to/file1.py", "relative/path/to/file2.js"],
        "summary_of_changes": "conventional commit message"
      }}
    - Only group files together if their changes are closely related; otherwise, create separate commits.
    - Do not include explanations or extra text—just valid JSON.
...
"""
    ...
    def _get_ai_suggestions(self, diff_content):
        ...
        prompt = self._generate_commit_prompt(diff_content)
